...
kubectl exec mongod-0 -c mongors -- mongo --eval 'rs.initiate({_id: "MainRepSet", version: 1, members: [ {_id: 0, host: "mongod-0.mongors.default.svc.cluster.local:27017"}, {_id: 1, host: "mongod-1.mongors.default.svc.cluster.local:27017"}, {_id: 2, host: "mongod-2.mongors.default.svc.cluster.local:27017"} ]});'

# Wait a bit until the replica set should have a primary ready
echo "Waiting for the Replica Set to initialise..."
sleep 30
kubectl exec mongod-0 -c mongors -- mongo --eval 'rs.status();'

# Create the admin user (this will automatically disable the localhost exception)
#echo "Creating user: 'main_admin'"
kubectl exec mongod-0 -c mongors -- mongo --eval 'db.getSiblingDB("admin").createUser({user:"main_admin",pwd:"'"${1}"'",roles:[{role:"root",db:"admin"}]});'
kubectl exec mongod-0 -c mongors -- mongo --eval 'db.getSiblingDB("IdentityManager").createUser({user:"identity",pwd:"'"${1}"'",roles:["readWrite"]})' -u main_admin -p "${1}" --authenticationDatabase admin
